...
export default class ScrapbookPlugin extends Plugin {
      ...
      if (line.startsWith('- [')) {
        ...
        while (j < lines.length && lines[j].trim().startsWith('  - ')) {
          ...
          if (fieldLine.includes('Importance:')) {
            const importance = fieldLine.split('Importance:')[1].trim() as 'High' | 'Medium' | 'Low';
  ...
  getCurrentQuestion(): Question | null {
    if (this.settings.currentQuestionId) {
      return this.questions.find(q => q.id === this.settings.currentQuestionId && !q.completed) || null;
    }
  ...
  getNextQuestion(): Question | null {
    ...
    const pending = this.questions.filter(q => !q.completed && q.id !== current?.id);
    
    // Sort by importance: High > Medium > Low
    const importanceOrder = { 'High': 3, 'Medium': 2, 'Low': 1 };
    return pending.sort((a, b) => importanceOrder[b.importance] - importanceOrder[a.importance])[0] || null;
  }
...
`;
  ...
  generateQuestionsMarkdown(): string {
    const active = this.questions.filter(q => !q.completed && q.id === this.settings.currentQuestionId);
    const pending = this.questions.filter(q => !q.completed && q.id !== this.settings.currentQuestionId);
    const completed = this.questions.filter(q => q.completed);
    
    // Sort by importance
    const importanceOrder = { 'High': 3, 'Medium': 2, 'Low': 1 };
    pending.sort((a, b) => importanceOrder[b.importance] - importanceOrder[a.importance]);
    ...
    active.forEach(q => {
      ...
      if (q.estimatedTime) content += `  - Estimated: ${this.formatTimeString(q.estimatedTime)}\n`;
      if (q.startTime) content += `  - Started: ${q.startTime.toISOString().slice(0, 16).replace('T', ' ')}\n`;
      content += `  - Time Spent: ${this.formatTimeString(this.calculateCurrentTimeSpent())}\n\n`;
    });
    ...
    pending.forEach(q => {
      ...
      if (q.estimatedTime) details.push(`Est: ${this.formatTimeString(q.estimatedTime)}`);
    ...
    completed.forEach(q => {
      content += `- [x] **${q.title}**`;
      const details: Array<String> = [];
      if (q.timeSpent > 0) details.push(`Time: ${this.formatTimeString(q.timeSpent)}`);
      if (q.estimatedTime) details.push(`${this.formatTimeString(q.estimatedTime)}`);
      if (details.length > 0) content += ` (${details.join('/')})`;
      content += '\n';
    });
  ...
  async startAPIServer() {
    ...
    this.server.listen(this.settings.apiPort, () => {
      console.log(`Scrapbook API server running on port ${this.settings.apiPort}`);
    });
  }
  ...
  async handleDocumentationRequest(docReq: DocumentationRequest) {
    ...
    if (docReq.image) {
      const imagePath = await this.saveImage(docReq.image.data, docReq.image.filename);
      content = `![[${imagePath}]]\n\n${content}`;
    }
