...
def create_shortcut(path_to_file: str, *parameters: str) -> dict:
    """
    ...
    """
    return lambda: subprocess.run([sys.executable, path_to_file, *parameters])
...
def start_listener(shortcuts_to_paths: dict, scrapbook_hyperkey: str):
    ...
    ```
    {
        "<alt>+<ctrl>+q": ("path/to/new_question.py", "1"),
    ...
    """
    shortcut_dict = {
        scrapbook_hyperkey + "+" + shortcut: create_shortcut(path, *args)
        for shortcut, (path, args) in shortcuts_to_paths.items()
    }
